...
class GridModel {
  ...
  isSolvable = (set) => {
    ...
    if (size % 2 === 1) {
      ...
      return (
        (inversions % 2 === 1 && fromBottom % 2 === 0) ||
        (inversions % 2 === 0 && fromBottom % 2 === 1)
      );
    }
  };
...
const checkValidMove = (position, tiles) => {
  ...
  if (Math.floor(blank / rowLength) === Math.floor(position / rowLength)) {
    //same row

    if (blank === position - 1 || blank === position + 1) return blank;
  } else {
    if (blank === position - rowLength || blank === position + rowLength)
      return blank;
  }
